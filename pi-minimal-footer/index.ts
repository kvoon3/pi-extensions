...
async function fetchCommandCodeUsage(): Promise<UsageSnapshot> {
  ...
  try {
    ...
    const whoamiRes = await fetchWithTimeout("https://api.commandcode.ai/alpha/whoami", { headers });
    if (!whoamiRes.ok) {
    ...
    const [creditsRes, summaryRes] = await Promise.all([
      fetchWithTimeout(`https://api.commandcode.ai/alpha/billing/credits${orgParam}`, { headers }),
      fetchWithTimeout(`https://api.commandcode.ai/alpha/usage/summary${orgParam}`, { headers }),
    ]);
